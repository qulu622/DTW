...
def mfm_dtw(ts1, ts2): # Minimum First Method for DTW
    cell_dict = dict()
    ...
    while (ts1_len-1, ts2_len-1) not in cell_dict:
        if (ts1_index+1, ts2_index+1) not in cell_dict and (ts1_index+1 < ts1_len) and (ts2_index+1 < ts2_len): #dia
...
def main():
    ...
    mfm_dtw_data = pd.DataFrame(columns=['NAME', 'TIME', 'CELL', 'ACCURACY'])
    mfm_dtw_data.to_csv('mfm_dtw_' + cur_dir + '.csv', index=0)
    ...
    for file in os.listdir(cur_dir + '/1/'):
        ...
        mfm_dtw_total_time = 0
        mfm_dtw_total_cell = 0
        mfm_dtw_matched = 0
        mfm_dtw_unmatched = 0
        ...
        for test_index, test_series in test_df.iterrows():
            ...
            for train_index, train_series in train_df.iterrows():
                mfm_dtw_start = time.time()
                mfm_dtw_distance, mfm_dtw_cell = mfm_dtw(test_series.to_list(), train_series.to_list())
                mfm_dtw_end = time.time()
                mfm_dtw_time = mfm_dtw_end - mfm_dtw_start

                mfm_dtw_total_time += mfm_dtw_time
                mfm_dtw_total_cell += mfm_dtw_cell

                if mfm_dtw_distance < minimum_distance:
                    minimum_distance = mfm_dtw_distance
        ...
        # print(mfm_dtw_total_time)
        # print(mfm_dtw_total_cell)

        mfm_dtw_data = mfm_dtw_data.append({'NAME': file, 'TIME': str(mfm_dtw_total_time), 'CELL': str(mfm_dtw_total_cell),
                            'ACCURACY': str(mfm_dtw_matched / (mfm_dtw_matched + mfm_dtw_unmatched))}, ignore_index=True)
        print(mfm_dtw_data)
        mfm_dtw_data.to_csv('mfm_dtw_' + cur_dir + '.csv', mode='a', header=0, index=0)
        mfm_dtw_data.drop(index=0, inplace=True)
    ...
    for file in os.listdir(cur_dir + '/10/'):
        ...
        for i in range(1, 11):
            ...
            mfm_dtw_data = mfm_dtw_data.append({'NAME': file, 'TIME': str(mfm_dtw_total_time), 'CELL': str(mfm_dtw_total_cell),
                                'ACCURACY': str(mfm_dtw_matched / (mfm_dtw_matched + mfm_dtw_unmatched))},
                               ignore_index=True)
            print(mfm_dtw_data)
            mfm_dtw_data.to_csv('mfm_dtw_' + cur_dir + '.csv', mode='a', header=0, index=0)
            mfm_dtw_data.drop(index=0, inplace=True)
