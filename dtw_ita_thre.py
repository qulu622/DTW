...
def dtw_ita_thre(ts1, ts2, line3_b, line4_b, minimum_distance): # DTW with Itk-band
    cell = 0
    ts1_len = len(ts1) - 1
    ts2_len = len(ts2) - 1
...
def main():
    ...
    dtw_ita_thre_data = pd.DataFrame(columns=['NAME', 'TIME', 'CELL', 'ACCURACY'])
    dtw_ita_thre_data.to_csv('dtw_ita_thre_' + cur_dir + '.csv', index=0)
    ...
    for file in os.listdir(cur_dir + '/1/'):
        ...
        dtw_ita_thre_total_time = 0
        dtw_ita_thre_total_cell = 0
        dtw_ita_thre_matched = 0
        dtw_ita_thre_unmatched = 0
        ...
        for test_index, test_series in test_df.iterrows():
            ...
            for train_index, train_series in train_df.iterrows():
                dtw_ita_thre_start = time.time()
                dtw_ita_thre_distance, dtw_ita_thre_cell = dtw_ita_thre(test_series.to_list(), train_series.to_list(), line3_b, line4_b, minimum_distance)
                dtw_ita_thre_end = time.time()
                dtw_ita_thre_time = dtw_ita_thre_end - dtw_ita_thre_start

                dtw_ita_thre_total_time += dtw_ita_thre_time
                dtw_ita_thre_total_cell += dtw_ita_thre_cell

                if dtw_ita_thre_distance < minimum_distance:
                    minimum_distance = dtw_ita_thre_distance
                    train_class = train_series.iat[0]

            # print(minimum_distance)
            if test_class == train_class:
                dtw_ita_thre_matched += 1
            else:
                dtw_ita_thre_unmatched += 1

        # print(dtw_ita_thre_total_time)
        # print(dtw_ita_thre_total_cell)

        dtw_ita_thre_data = dtw_ita_thre_data.append({'NAME': file, 'TIME': str(dtw_ita_thre_total_time), 'CELL': str(dtw_ita_thre_total_cell),
                            'ACCURACY': str(dtw_ita_thre_matched/(dtw_ita_thre_matched+dtw_ita_thre_unmatched))}, ignore_index=True)
        print(dtw_ita_thre_data)
        dtw_ita_thre_data.to_csv('dtw_ita_thre_' + cur_dir + '.csv', mode='a', header=0, index=0)
        dtw_ita_thre_data.drop(index=0, inplace=True)
