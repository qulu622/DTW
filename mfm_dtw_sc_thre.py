...
def mfm_dtw_sc_thre(ts1, ts2, minimum_distance): # Minimum First Method for DTW with SC-band
    cell_dict = dict()
...
def main():
    ...
    mfm_dtw_sc_thre_data = pd.DataFrame(columns=['NAME', 'TIME', 'CELL', 'ACCURACY'])
    mfm_dtw_sc_thre_data.to_csv('mfm_dtw_sc_thre_' + cur_dir + '.csv', index=0)
    ...
    for file in os.listdir(cur_dir + '/1/'):
        ...
        mfm_dtw_sc_thre_total_time = 0
        mfm_dtw_sc_thre_total_cell = 0
        mfm_dtw_sc_thre_matched = 0
        mfm_dtw_sc_thre_unmatched = 0
        ...
        for test_index, test_series in test_df.iterrows():
            ...
            for train_index, train_series in train_df.iterrows():
                mfm_dtw_sc_thre_start = time.time()
                mfm_dtw_sc_thre_distance, mfm_dtw_sc_thre_cell = mfm_dtw_sc_thre(test_series.to_list(), train_series.to_list(), minimum_distance)
                mfm_dtw_sc_thre_end = time.time()
                mfm_dtw_sc_thre_time = mfm_dtw_sc_thre_end - mfm_dtw_sc_thre_start

                mfm_dtw_sc_thre_total_time += mfm_dtw_sc_thre_time
                mfm_dtw_sc_thre_total_cell += mfm_dtw_sc_thre_cell

                if mfm_dtw_sc_thre_distance < minimum_distance:
                    minimum_distance = mfm_dtw_sc_thre_distance
                    train_class = train_series.iat[0]

            # print(minimum_distance)
            if test_class == train_class:
                mfm_dtw_sc_thre_matched += 1
            else:
                mfm_dtw_sc_thre_unmatched += 1

        # print(mfm_dtw_sc_thre_total_time)
        # print(mfm_dtw_sc_thre_total_cell)

        mfm_dtw_sc_thre_data = mfm_dtw_sc_thre_data.append({'NAME': file, 'TIME': str(mfm_dtw_sc_thre_total_time), 'CELL': str(mfm_dtw_sc_thre_total_cell),
                            'ACCURACY': str(mfm_dtw_sc_thre_matched / (mfm_dtw_sc_thre_matched + mfm_dtw_sc_thre_unmatched))},
                           ignore_index=True)
        print(mfm_dtw_sc_thre_data)
        mfm_dtw_sc_thre_data.to_csv('mfm_dtw_sc_thre_' + cur_dir + '.csv', mode='a', header=0, index=0)
        mfm_dtw_sc_thre_data.drop(index=0, inplace=True)
